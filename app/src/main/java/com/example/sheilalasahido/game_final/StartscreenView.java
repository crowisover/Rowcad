...
public class StartscreenView extends View{
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if(event.getAction() == MotionEvent.ACTION_DOWN) {
            if((event.getX() > REGION_PLAY[0] * getWidth()) && (event.getX() < REGION_PLAY[2] * getWidth())
                    && (event.getY() > REGION_PLAY[1] * getHeight())
                    && (event.getY() < REGION_PLAY[3] * getHeight())) {
                mainActivity.startActivity(new Intent("com.example.sheilalasahido.game_final.Game"));
                ...
                mainActivity.startActivity(new Intent("com.example.sheilalasahido.game_final.About"));
            }
        }
        return true;
    }
